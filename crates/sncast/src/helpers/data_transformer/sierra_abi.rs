use crate::helpers::data_transformer::transformer::{
    AllowedCalldataArguments, ArgumentName, CalldataArrayMacro, CalldataEnum,
    CalldataSingleArgument, CalldataStruct, CalldataStructField, CalldataTuple,
};
...
pub(crate) fn parse_expr(
    ...
    abi: &Vec<AbiEntry>,
    db: &SimpleParserDatabase,
) -> anyhow::Result<AllowedCalldataArguments> {
    match expr {
        Expr::StructCtorCall(expr_struct_ctor_call) => {
            ...
            let fields = struct_args_with_values
                ...
                .map(|(arg_name, expr)| {
                    ...
                    Ok(CalldataStructField::new(
                        parse_expr(expr, abi_entry.r#type.clone(), abi, db)?,
        ...
        Expr::InlineMacro(expr_inline_macro) => {
            ...
            let abi_argument_type = array_elem_type_regex
                ...
                .context(format!(
                    r#"Invalid argument type, expected "{}", got array"#,
                    param_type
                ))?
                .get(1)
                // TODO better message
                .context(format!(
                    "Couldn't parse array element type from the ABI array parameter: {}",
                    param_type
                ))?
                .as_str();
